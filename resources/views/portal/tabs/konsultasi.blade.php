<div x-data="chatbotApp()" class="bg-white border border-gray-100 rounded-2xl shadow-sm w-full flex flex-col h-[calc(100vh-160px)] overflow-hidden dark:bg-[#1E293B] dark:border-gray-800">
    ...
    <div class="p-6 border-b border-gray-100 flex items-center justify-between bg-white dark:bg-[#1E293B] dark:border-gray-800 flex-shrink-0 shadow-sm z-10">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-md transition-all duration-300"
                ...
                <svg aria-hidden="true" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
            </div>
            <div>
    ...
    <div id="chat-container" class="flex-grow p-6 md:p-8 overflow-y-auto chat-scroll bg-gray-50/30 dark:bg-[#0B1120]">
        ...
        <div class="bg-teal-50 border border-teal-100/50 rounded-xl p-5 text-sm text-teal-800 mb-8 flex items-start gap-3.5 dark:bg-teal-900/20 dark:border-teal-800 dark:text-teal-400 max-w-3xl mx-auto shadow-sm">
            <svg aria-hidden="true" class="w-5 h-5 flex-shrink-0 mt-0.5 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <div>
                    ...
                    <template x-if="msg.sender === 'bot' || msg.sender === 'admin'">
                        <div class="flex items-center gap-1.5 mb-1.5 pl-1.5">
                            <div class="w-5 h-5 rounded-md bg-teal-100 text-teal-600 flex items-center justify-center dark:bg-teal-900/50 dark:text-teal-400">
                                <svg aria-hidden="true" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                            </div>
                    ...
                    <div :class="msg.sender === 'bot' || msg.sender === 'admin'
                        ...
                        <template x-if="msg.type === 'form' && step === 1">
                            <div class="mt-4 bg-gray-50 border border-gray-200 rounded-xl p-5 w-full sm:w-[480px] dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        <label for="durasi" class="block text-[11px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-1.5">Sejak kapan dirasakan?</label>
                                        <input type="text" id="durasi" x-model="formKeluhan.durasi" placeholder="Contoh: Sejak 2 hari yang lalu" class="w-full text-xs p-3 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                    </div>

                                    <div>
                                        <label for="reaksi_obat" class="block text-[11px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-1.5">Obat yang diminum & reaksi?</label>
                                        <input type="text" id="reaksi_obat" x-model="formKeluhan.reaksi_obat" placeholder="Contoh: Paracetamol, mual mereda" class="w-full text-xs p-3 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                    </div>

                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <label for="alergi" class="block text-[11px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-1.5">Riwayat alergi obat</label>
                                            <input type="text" id="alergi" x-model="formKeluhan.alergi" placeholder="Contoh: Penisilin / tidak ada" class="w-full text-xs p-3 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                        </div>

                                        <div>
                                            <label for="gejala_lain" class="block text-[11px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-1.5">Gejala penyerta lainnya</label>
                                            <input type="text" id="gejala_lain" x-model="formKeluhan.gejala_lain" placeholder="Contoh: Mual, demam tinggi" class="w-full text-xs p-3 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                        </div>
                                    </div>

                                    <div>
                                        <label for="riwayat_penyakit" class="block text-[11px] font-black text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-1.5">Riwayat penyakit terdahulu</label>
                                        <input type="text" id="riwayat_penyakit" x-model="formKeluhan.riwayat_penyakit" placeholder="Contoh: Asma, Hipertensi" class="w-full text-xs p-3 bg-white border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                                    </div>
    ...
    <div class="p-6 bg-white border-t border-gray-100 dark:bg-[#1E293B] dark:border-gray-800 flex-shrink-0">
        <div class="max-w-3xl mx-auto">
            <form @submit.prevent="sendMessage" class="relative flex items-center">
                <input type="text" x-model="newMessage" :disabled="step === 1" aria-label="Tulis keluhan atau pertanyaan Anda"
                ...
                <button type="submit" :disabled="newMessage.trim() === '' || step === 1" aria-label="Kirim Pesan"
                    ...
                    <svg aria-hidden="true" class="w-4 h-4 translate-x-[-1px] translate-y-[1px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                </button>
            </form>
